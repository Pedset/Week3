...
public class LoginDialog extends JFrame {
    ...
    private void enableLoginButton() {
        new Thread(() -> {
            ...
            SwingUtilities.invokeLater(
                    () -> btnLogin.setEnabled(tfUsername.getText().matches(".*\\S.*") && String.valueOf(pfPassword.getPassword()).matches(".*\\S.*")));
        }).start();
        ;
    }
    ...
    protected void onInvalidCredentials() {
        ...
        if (!tfUsername.getText().equals("bob")){
        	tfUsername.setBackground(Color.red);
        	pfPassword.setBackground(Color.red);
        }
        else if (!pfPassword.getPassword().equals("secret")) {
        	pfPassword.setBackground(Color.red);
        }
